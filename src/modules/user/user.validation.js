...
const signupVal= joi.object({
    ...
    password:joi.string().min(7).max(40).pattern(/^[^A-Z][A-Za-z0-9]{7,39}$/).required(),
...
const signinVal= joi.object({
    email:joi.string().email().required(),
    password:joi.string().min(7).max(40).pattern(/^[^A-Z][A-Za-z0-9]{7,39}$/).required().messages({
...
const changePasswordVal= joi.object({
    email:joi.string().email().required(),
    oldPassword:joi.string().min(7).max(40).pattern(/^[^A-Z][A-Za-z0-9]{7,39}$/).required().messages({
        ...
        newPassword:joi.string().min(7).max(40).pattern(/^[^A-Z][A-Za-z0-9]{7,39}$/).required().messages({
